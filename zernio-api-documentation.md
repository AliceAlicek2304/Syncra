...
- **Authentication:** Bearer token → paste your API key (the full `sk_...` string).
...
**$2 per WhatsApp number / month**, separate from the social-account price. WhatsApp uses Meta's WhatsApp Cloud API, which has its own number-based pricing model.
